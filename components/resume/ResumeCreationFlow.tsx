...
function MiniTemplate({ settings }: { settings: ResumeSettings }) {
  return <div className="relative h-[285px] overflow-hidden rounded-[12px] bg-[#f4f5f7]"><div className="absolute left-1/2 top-3 origin-top -translate-x-1/2 scale-[0.29] shadow-[0_14px_30px_-16px_rgba(15,23,42,.45)]"><ResumeDocument data={EMPTY_RESUME_DATA} settings={settings} /></div></div>;
}
...
function TemplateSelection({ current, onBack, onContinue }: { current: TemplateId; onBack: () => void; onContinue: (id: TemplateId) => void }) {
  ...
  return <div className="oe-template min-h-dvh bg-[#fafafa] text-[#111827]"><header className="flex h-[68px] items-center justify-center border-b border-[#ececef] bg-white"><img src="/logo-orricaedge.png" alt="Orrica Edge" className="h-[30px] w-auto" /></header><main className="mx-auto max-w-7xl px-4 py-8 sm:px-7 sm:py-10 lg:px-10"><div className="mx-auto max-w-3xl text-center"><span className="text-[11px] font-extrabold uppercase tracking-[.14em] text-[#f47c3c]">Step 1</span><h1 className="mt-2 text-[30px] font-extrabold tracking-[-.035em] sm:text-[44px]">Select a template</h1><p className="mt-3 text-[13px] leading-6 text-[#667085] sm:text-[15px]">Choose a professional design from our template library. You can change it later without losing your resume data.</p></div><div className="mx-auto mt-8 grid max-w-6xl grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{TEMPLATES.map(template=>{const active=selected===template.id;const previewSettings={...DEFAULT_SETTINGS,template:template.id};return <button key={template.id} type="button" onClick={()=>setSelected(template.id)} className={`group relative overflow-hidden rounded-2xl border-2 bg-white p-2 text-left transition-all hover:-translate-y-0.5 hover:shadow-lg ${active?'border-[#f47c3c] shadow-[0_12px_30px_-18px_rgba(244,124,60,.9)]':'border-[#e5e7eb]'}`}>{template.tag&&<span className="absolute left-4 top-4 z-20 rounded-full bg-[#f47c3c] px-2.5 py-1 text-[9px] font-extrabold tracking-[.08em] text-white">{template.tag}</span>}{active&&<span className="absolute right-4 top-4 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-[#f47c3c] text-white shadow"><Check className="h-4 w-4" /></span>}<MiniTemplate settings={previewSettings}/><div className="px-3 pb-3 pt-3"><h2 className="text-[15px] font-extrabold text-[#230939]">{template.name}</h2><p className="mt-1 text-[11px] leading-5 text-[#667085]">{template.description}</p></div></button>})}</div><div className="mx-auto mt-7 flex max-w-6xl items-center justify-between border-t border-[#e7e9ee] pt-5"><button type="button" onClick={onBack} className="inline-flex h-11 items-center gap-1.5 rounded-xl border border-[#e5e7eb] bg-white px-4 text-[13px] font-bold text-[#230939]"><ArrowLeft className="h-4 w-4"/> Back</button><button type="button" onClick={()=>onContinue(selected)} className="inline-flex h-11 min-w-[170px] items-center justify-center gap-2 rounded-xl bg-[#f47c3c] px-5 text-[13px] font-bold text-white hover:bg-[#e5682e]">Continue <ArrowRight className="h-4 w-4"/></button></div></main><style jsx global>{`.oe-template,.oe-template *{font-family:"Proxima Nova",Arial,sans-serif}.oe-template button{touch-action:manipulation}`}</style></div>;
}

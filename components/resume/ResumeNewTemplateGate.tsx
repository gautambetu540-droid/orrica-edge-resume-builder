...
function isValidTemplate(id: string | null): id is TemplateId {
  return Boolean(id && TEMPLATE_LIST.some((template) => template.id === id));
}
...
function saveTemplate(id: TemplateId) {
  try {
    ...
    localStorage.setItem('orrica_edge_draft_v1', JSON.stringify({
      ...
      settings: { ...DEFAULT_SETTINGS, ...(parsed.settings ?? {}), template: id },
    }));
...
function setTemplateUrl(id: TemplateId) {
  const url = new URL(window.location.href);
  url.pathname = '/resume/new';
  ...
  return (
    <div className="oe-create-shell min-h-dvh bg-white text-[#171310]">
      ...
      <main className="mx-auto grid max-w-[1180px] items-center gap-12 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[.85fr_1.15fr] lg:gap-16 lg:py-20">
        <section>
          ...
          <div className="mt-8 space-y-5">
            {[
              ...
              <div key={num} className="flex gap-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-orange-500 text-xs font-black text-white shadow-[0_10px_25px_-14px_rgba(249,115,22,.9)]">{num}</span><div><h2 className="text-base font-extrabold text-[#171310] sm:text-lg">{title}</h2><p className="mt-1 text-sm leading-6 text-[#75675D]">{text}</p></div></div>
            ))}
          </div>
        ...
        <section className="relative mx-auto w-full max-w-[620px]"><div className="relative rounded-[28px] border border-orange-100 bg-[#FFF8F2] p-5 shadow-[0_35px_80px_-45px_rgba(70,35,10,.35)] sm:p-8"><div className="mb-4 flex items-center justify-between"><div className="flex gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-orange-200"/><span className="h-2.5 w-2.5 rounded-full bg-orange-300"/><span className="h-2.5 w-2.5 rounded-full bg-orange-400"/></div><span className="rounded-full bg-white px-3 py-1 text-[9px] font-bold text-orange-700 shadow-sm">LIVE PREVIEW</span></div><div className="rounded-xl bg-white p-5 shadow-[0_20px_45px_-30px_rgba(60,30,10,.35)] sm:p-7"><div className="flex items-start justify-between border-b border-orange-100 pb-4"><div><div className="text-xl font-black tracking-tight text-[#171310]">Alex Morgan</div><div className="mt-1 text-[9px] font-semibold text-orange-600">MARKETING SPECIALIST</div></div><div className="h-10 w-10 rounded-full border-4 border-orange-100 bg-orange-50"/></div><div className="mt-5 space-y-4">{['PROFESSIONAL SUMMARY','EXPERIENCE','EDUCATION','SKILLS'].map((heading) => <div key={heading}><div className="text-[8px] font-black tracking-[.14em] text-orange-600">{heading}</div><div className="mt-1.5 h-1.5 rounded-full bg-neutral-100"/></div>)}</div></div><div className="mt-4 flex items-center justify-between rounded-xl border border-orange-100 bg-white px-4 py-3"><span className="text-[10px] font-bold text-[#6B5A4E]">ATS-friendly · A4-ready · Editable</span><span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-orange-600"><Check className="h-3.5 w-3.5"/> Ready to build</span></div></div></section>
      </main>
    </div>
  );
}
...
export default function ResumeNewTemplateGate() {
  ...
  // Deep links such as /resume/new?template=fresher-03 now open the selected
  // template directly. The URL, local draft and editor all use the same ID.
  useEffect(() => {

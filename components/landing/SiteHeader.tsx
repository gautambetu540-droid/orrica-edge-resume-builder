  ...
  return (
    <>
      ...
      <header className="oe-glass-nav sticky top-0 z-50">
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between px-6 lg:px-8 xl:px-10">
          <Link href="/" className="group shrink-0" aria-label="Orrica Edge home">
            <img src="/logo-orricaedge.png" alt="Orrica Edge AI Resume Builder" className="h-7 w-auto transition-transform duration-300 group-hover:scale-[1.035] group-hover:-rotate-1 sm:h-8" />
          </Link>
          ...
          <div className="hidden shrink-0 items-center gap-3 lg:flex">
            <a href={JOB_CHANNEL} target="_blank" rel="noreferrer" className="whitespace-nowrap rounded-xl px-3.5 py-2.5 text-[14px] font-medium text-neutral-600 transition-all hover:bg-orange-50 hover:text-orange-700">Free jobs</a>
            <Link href="/login">
              <Button variant="ghost" className="h-11 whitespace-nowrap rounded-xl px-4 text-[14px] font-medium text-neutral-600 transition-all hover:bg-neutral-100 hover:text-neutral-950">Sign in</Button>
            </Link>
            <Link href="/resume/new">
              <Button className="oe-3d-button magnetic-button h-12 whitespace-nowrap rounded-[14px] px-6 text-[14px] font-bold shadow-lg shadow-orange-500/20">
                Create resume <ArrowUpRight className="ml-1.5 h-3.5 w-3.5" />
              </Button>
            </Link>
          </div>

          <button className="rounded-xl p-2.5 text-neutral-700 hover:bg-neutral-100 lg:hidden" onClick={() => setMenuOpen((value) => !value)} aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen}>
            {menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </button>
        </div>

        {menuOpen && (
          <div className="animate-fade-in-up border-t border-black/[0.06] bg-white/95 px-5 py-4 shadow-2xl backdrop-blur-xl lg:hidden">
            <nav className="flex flex-col">
              {NAV_LINKS.map((link, index) => (
                <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)} className={`reveal-${Math.min(index + 1, 6)} animate-fade-in-up border-b border-neutral-100 py-3.5 text-sm font-semibold text-neutral-700 last:border-0`}>{link.label}</Link>
              ))}
            </nav>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <Link href="/login" onClick={() => setMenuOpen(false)}><Button variant="outline" className="h-11 w-full rounded-xl">Sign in</Button></Link>
              <Link href="/resume/new" onClick={() => setMenuOpen(false)}><Button className="oe-3d-button magnetic-button h-11 w-full rounded-xl">Create resume</Button></Link>
            </div>
          </div>
        )}
      </header>

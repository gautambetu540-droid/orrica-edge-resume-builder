...
export interface ResumeDocumentProps {
  ...
  /** Set true only for the dedicated print/PDF route — disables shadow/border chrome. */
  forPrint?: boolean;
}
...
export function ResumeDocument({ data, settings, forPrint = false }: ResumeDocumentProps) {
  ...
  const style: React.CSSProperties & Record<string, string> = {
    ...
    '--entry-gap': `${Math.max(8, settings.sectionSpacing * 0.6)}px`,
    fontFamily: FONT_STACKS[settings.font],
    fontSize: `${settings.fontSize}pt`,
    lineHeight: settings.lineSpacing,
    // Two-column templates handle their own inset per-column (so the
    // sidebar can bleed to the page edge); single-column templates use a
    // uniform margin on the whole page.
    padding: preset.layout === 'two-column' ? 0 : `${settings.margin}mm`,
    color: '#1f2933',
  };

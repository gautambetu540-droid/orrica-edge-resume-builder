...
function ContactLine({ info, inverse = false }: { info: PersonalInfo; inverse?: boolean }) {
  ...
  return (
    <div className={`flex flex-wrap gap-x-2 gap-y-0.5 leading-snug text-[0.84em] ${inverse ? 'text-white/90' : 'text-[var(--resume-muted)]'}`}>
      {parts.map((p, i) => (
        <span key={i} className="flex items-center gap-2 break-all">
          {p}
          {i < parts.length - 1 && <span className={inverse ? 'text-white/40' : 'text-neutral-300'}>|</span>}
        </span>
      ))}
    </div>
  );
    ...
    return (
      <div className="mb-6 border-b-2 pb-4" style={{ borderColor: 'var(--accent)' }}>
        <div className={`${centered ? 'text-center' : ''}`}>
          ...
          {title && <p className="mt-1 font-medium" style={{ color: 'var(--accent)', fontSize: '1.02em' }}>{title}</p>}
        </div>
      ...
      <div className="mb-4 border-t-[5px] pt-4" style={{ borderColor: 'var(--accent)' }}>
        <div className="flex items-end justify-between gap-5">
          <div className="min-w-0">
            ...
            {title && <p className="mt-1 font-medium text-[var(--resume-muted)]" style={{ fontSize: '0.98em' }}>{title}</p>}
          </div>
          <div className="max-w-[58%] text-right"><ContactLine info={info} /></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex gap-4 mb-5 ${centered ? 'flex-col items-center text-center' : 'items-center justify-between flex-wrap'}`}>
      <div className={centered ? '' : 'flex items-center gap-4'}>
        ...
        <div>
          <h1 className="font-bold leading-[1.05] tracking-[-0.035em]" style={{ fontSize: 'calc(1.9em * var(--heading-scale))', color: '#111827' }}>
            {info.fullName || 'Your Name'}
          </h1>
          {title && <p className="font-medium mt-1" style={{ color: 'var(--accent)', fontSize: '1.02em' }}>{title}</p>}
        </div>
      </div>

...
export function TwoColumnLayout({
  ...
  const ordered = [...settings.sections].filter((s) => s.visible && s.id !== 'header').sort((a, b) => a.order - b.order);
  const sidebar = ordered.filter((s) => SIDEBAR_SECTIONS.includes(s.id) && !isSectionEmpty(s.id, data));
  const main = ordered.filter((s) => !SIDEBAR_SECTIONS.includes(s.id) && !isSectionEmpty(s.id, data));
  const info = data.personalInfo;
  ...
  return (
    <div className="flex" style={{ minHeight: '100%' }}>
      ...
      >
        ...
        <h1 className="text-[1.4em] font-bold leading-tight mb-0.5">{info.fullName || 'Your Name'}</h1>
        {info.professionalTitle && <p className="text-[0.95em] text-white/85 mb-4">{info.professionalTitle}</p>}
      ...
      <main className="flex-1 px-6 py-6 flex flex-col" style={{ gap: 'var(--section-gap)' }}>
        {main.map((section) => (

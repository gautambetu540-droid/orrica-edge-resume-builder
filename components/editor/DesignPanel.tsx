        ...
        <div className="grid grid-cols-2 gap-3">
          {TEMPLATE_LIST.map((t) => (
            <button key={t.id} type="button" onClick={() => applyTemplate(t)} className={`relative rounded-lg border-2 p-3 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${settings.template === t.id ? 'border-primary bg-accent shadow-sm' : 'border-border hover:border-neutral-300'}`}>
              ...
              <div className="mb-2 aspect-[3/4] overflow-hidden rounded border bg-white" style={{ pointerEvents: 'none' }}><TemplatePreviewCard accent={t.defaultAccentColor} layout={t.layout} headerVariant={t.headerVariant} sidebarVariant={t.sidebarVariant} font={t.recommendedFont} compact /></div>
              <p className="text-xs font-semibold">{t.name}</p><p className="mt-0.5 line-clamp-2 text-[10px] leading-4 text-muted-foreground">{t.description}</p>
            </button>
          ))}
        </div>
      </section>

...
export function OrricaResumeWizard({ data, settings, updateData, updateSettings, onFinish, finishing }: StepProps & { onFinish: () => void; finishing?: boolean }) {
  ...
  return <div className="oe-resume-wizard h-[100dvh] min-h-0 overflow-hidden bg-white text-slate-900">
    <aside className="oe-resume-sidebar fixed inset-y-0 left-0 z-40 hidden w-[238px] overflow-hidden border-r border-orange-100 bg-white px-4 py-5 md:block"><div className="flex h-full min-h-0 flex-col"><div className="px-2 text-[10px] font-bold uppercase tracking-[.14em] text-orange-600">Build your resume</div><div className="relative mt-5 space-y-1"><div className="absolute left-[27px] top-5 bottom-5 w-px bg-orange-100" />{STEPS.map(([label, description], i) => { const active = i === index; const complete = i < index; return <button key={label} type="button" disabled={i > index} onClick={() => i <= index && setIndex(i)} className={`relative z-10 flex w-full items-center gap-3 rounded-xl px-2.5 py-2.5 text-left ${active ? 'bg-orange-50' : 'hover:bg-orange-50'} disabled:cursor-default disabled:opacity-60`}><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 text-[11px] font-bold ${active ? 'border-orange-500 bg-orange-500 text-white' : complete ? 'border-orange-500 bg-white text-orange-600' : 'border-slate-300 bg-white text-slate-500'}`}>{complete ? <Check className="h-3.5 w-3.5" /> : i + 1}</span><span className="min-w-0"><strong className={`block text-[13px] leading-4 ${active ? 'text-slate-900' : 'text-slate-600'}`}>{label}</strong><small className="mt-0.5 block truncate text-[10px] text-slate-400">{description}</small></span></button>; })}</div><div className="mt-6 border-t border-orange-100 pt-4"><div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[.08em] text-slate-400"><span>Resume completeness</span><span className="text-orange-600">{completion}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-orange-100"><div className="h-full rounded-full bg-orange-500" style={{ width: `${Math.max(2, completion)}%` }} /></div></div><nav className="mt-auto space-y-1.5 border-t border-orange-100 pt-4 text-[10px] font-semibold text-slate-500"><a href="/terms" className="block hover:text-orange-600">Terms & Conditions</a><a href="/privacy" className="block hover:text-orange-600">Privacy Policy</a><a href="/accessibility" className="block hover:text-orange-600">Accessibility</a><a href="/about" className="block hover:text-orange-600">Contact Us</a><div className="pt-1 text-[9px] font-normal text-slate-400">© 2026 Orrica Edge</div></nav></div></aside>

    <div className="oe-resume-workspace h-[100dvh] min-h-0 min-w-0 overflow-y-auto overflow-x-hidden md:ml-[238px]"><header className="sticky top-0 z-30 flex h-[64px] shrink-0 items-center border-b border-orange-100 bg-white/95 px-4 backdrop-blur sm:px-6"><a href="/" className="shrink-0" aria-label="Orrica Edge home"><img src="/logo-orricaedge.png" alt="Orrica Edge" className="h-[27px] w-auto" /></a><div className="ml-4 hidden h-6 w-px bg-orange-100 sm:block" /><div className="ml-4 hidden min-w-0 sm:block"><div className="text-xs font-bold text-slate-900">Resume Builder</div><div className="text-[10px] text-slate-400">Build your resume step by step</div></div><div className="ml-auto flex items-center gap-3 text-[11px] text-slate-500 sm:gap-5"><span className="hidden items-center gap-1.5 font-semibold text-emerald-600 sm:flex"><ShieldCheck className="h-4 w-4" /> Saved privately</span><span>Step {index + 1} of {STEPS.length}</span><a href="/" className="font-bold text-slate-800 hover:text-orange-600">Exit</a></div></header>
      <div className="oe-resume-workspace-content grid min-w-0 grid-cols-1 items-start lg:grid-cols-[minmax(0,1fr)_400px]"><main className="min-w-0 bg-white"><div className="px-4 pb-6 pt-6 sm:px-8 sm:pt-9 lg:px-10"><div className="mx-auto w-full max-w-[820px]"><button type="button" onClick={back} disabled={index === 0} className="mb-5 inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-orange-600 disabled:invisible"><ArrowLeft className="h-3.5 w-3.5" /> Go Back</button><div className="text-[10px] font-bold uppercase tracking-[.14em] text-orange-600">{step[0]}</div><h1 className="mt-2 max-w-[760px] text-[27px] font-bold leading-[1.12] tracking-[-.025em] text-slate-900 sm:text-[34px]">{HEADINGS[index]}</h1><p className="mt-2 max-w-[680px] text-[14px] leading-6 text-slate-500 sm:text-[15px]">{SUBTITLES[index]}</p>{index === 0 && <p className="mt-3 text-[11px] text-slate-400">* indicates a required field</p>}<div className="mt-6 pb-2">{StepContent({ index, props: { data, settings, updateData, updateSettings } })}</div></div></div><div className="flex min-h-[70px] items-center gap-2 border-t border-orange-100 bg-white px-4 py-4 sm:px-8 lg:px-10"><div className="mx-auto flex w-full max-w-[820px] items-center gap-2"><Button variant="ghost" onClick={back} disabled={index === 0} className="h-11 rounded-lg border border-orange-100 px-4 font-bold text-slate-800 hover:bg-orange-50"><ArrowLeft className="mr-1.5 h-4 w-4" /> Back</Button><div className="flex-1" />{index > 0 && !last && <Button variant="outline" onClick={next} disabled={finishing} className="h-11 rounded-lg border border-orange-100 px-5 font-bold text-slate-800 hover:border-orange-400 hover:bg-orange-50">Skip</Button>}<Button onClick={next} disabled={finishing} className="h-11 min-w-[138px] rounded-lg bg-orange-500 px-5 font-bold text-white shadow-none hover:bg-orange-600">{last ? (finishing ? 'Saving…' : 'Download Resume') : <>Next: {STEPS[index + 1]?.[0] ?? 'Finish'} <ArrowRight className="ml-1.5 h-4 w-4" /></>}</Button></div></div></main><aside className="oe-resume-preview min-w-0 border-t border-orange-100 bg-[#FFF9F5] px-3 py-5 sm:px-5 lg:border-l lg:border-t-0"><PagePreview data={data} settings={settings} activeSection={step[2]} /></aside></div>
    </div>
    ...
    <style jsx global>{`
      ...
      .oe-resume-wizard input:focus,.oe-resume-wizard textarea:focus,.oe-resume-wizard select:focus{border-color:${ORANGE}!important;box-shadow:0 0 0 3px rgba(249,115,22,.12)!important;outline:none!important}

...
export function WizardShell({ data, settings, updateData, updateSettings, onFinish, finishing }: StepProps & { onFinish: () => void; finishing?: boolean }) {
  ...
  function goBack() { setStepIndex((current) => Math.max(0, current - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  ...
  return (
    <div className="oe-wizard-shell min-h-dvh overflow-x-hidden text-neutral-950">
      ...
      <header className="oe-glass-nav sticky top-0 z-40">
        <div className="mx-auto flex h-[60px] max-w-7xl items-center gap-2.5 px-3 sm:h-16 sm:gap-4 sm:px-6 lg:px-8">
          <a href="/" className="shrink-0" aria-label="Orrica Edge home"><img src="/logo-orricaedge.png" alt="Orrica Edge" className="h-6 w-auto sm:h-7" /></a>
          <div className="hidden h-5 w-px bg-neutral-200 sm:block" />
          <div className="min-w-0 flex-1"><div className="truncate text-[11px] font-semibold text-neutral-900 sm:text-xs">Create your resume</div><div className="hidden truncate text-[10px] text-neutral-400 sm:block">Build, scan, choose a template and preview everything before saving.</div></div>
          <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3">
            {accountName ? <div className="oe-glass flex max-w-[158px] items-center gap-1.5 rounded-full px-1.5 py-1 sm:max-w-[220px] sm:gap-2 sm:px-2.5 sm:py-1.5" title={accountName}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-orange-50 text-orange-700"><UserRound className="h-3.5 w-3.5" /></span><div className="min-w-0 leading-none"><div className="truncate text-[10px] font-bold text-neutral-800 sm:text-[11px]">{accountName}</div><div className="mt-0.5 text-[8px] font-medium text-emerald-600 sm:text-[9px]">Signed in</div></div></div> : <div className="hidden items-center gap-2 text-[10px] font-medium text-neutral-400 sm:flex"><ShieldCheck className="h-3.5 w-3.5 text-emerald-600" /> Private by default</div>}
          </div>
        </div>
      </header>

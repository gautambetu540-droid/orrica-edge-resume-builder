...
export function AuthForm({ mode = 'signin', returnTo: defaultReturnTo = '/dashboard' }: { mode?: AuthMode; returnTo?: string }) {
  ...
  const returnTo = params.get('returnTo') || defaultReturnTo;
      ...
      <div className="relative mx-auto grid min-h-[calc(100dvh-4rem)] max-w-6xl items-center gap-10 lg:grid-cols-[1fr_440px]">
        ...
        <section className="animate-scale-in">
          ...
          <div className="overflow-hidden rounded-[28px] border border-white/10 bg-white text-neutral-950 shadow-[0_35px_100px_-45px_rgba(0,0,0,.8)]">
            ...
            <div className="p-6 sm:p-7">{welcomeName && <div className="mb-5 animate-fade-in-up rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><strong>Welcome, {welcomeName}.</strong> Your account is ready.</div>}
              <Button variant="outline" className="h-11 w-full rounded-xl border-neutral-200" onClick={handleGoogle}><svg className="h-4 w-4" viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" /><path fill="#34A853" d="M12 23c2.97 0 5.46-0.98 7.28-2.66l-3.57-2.77c-0.98 0.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" /><path fill="#EA4335" d="M12 5.38c1.62 0 3.06 0.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c0.87-2.6 3.3-4.53 6.16-4.53z" /></svg>Continue with Google</Button>

        ...
        <div className="grid min-h-[470px] grid-cols-[132px_1fr] bg-neutral-50 sm:grid-cols-[170px_1fr]">
          ...
          <div className="grid grid-cols-[1fr_.82fr] gap-3 p-3 sm:gap-4 sm:p-4">
            <div className="rounded-xl border border-neutral-100 bg-white p-3.5 shadow-sm sm:p-4">
              ...
              {['Full name', 'Professional title', 'Email address', 'Phone number'].map((label, index) => <div key={label} className="mb-2.5"><div className="mb-1 text-[7px] font-bold text-neutral-500">{label}</div><div className="h-7 rounded-md border border-neutral-200 bg-white px-2 py-2 text-[8px] text-neutral-700">{['John Doe', 'Product Designer', 'john@example.com', '+91 98765 43210'][index]}</div></div>)}
...
function TypingHeadline() {
  return (
    <h1 className="animate-fade-in-up reveal-1 text-balance mt-6 text-4xl font-semibold tracking-[-0.055em] text-neutral-950 sm:text-6xl lg:text-[76px] lg:leading-[.98]" aria-label="Build a resume that gets remembered.">
      Build a resume that<br className="hidden sm:block" />{' '}
      <span className="typing-word text-gradient-brand">gets remembered.<span className="typing-caret" aria-hidden="true" /></span>
    </h1>
  );
}
...
export default function LandingPage() {
  return (
    <div className="min-h-dvh overflow-x-hidden bg-white">
      ...
      <SiteFooter />
      <style jsx global>{`
        .typing-word{position:relative;display:inline-block;white-space:nowrap}
        .typing-caret{display:inline-block;width:3px;height:.88em;margin-left:7px;vertical-align:-.05em;background:#f97316;border-radius:999px;animation:typing-caret .8s steps(1,end) infinite}
        @keyframes typing-caret{0%,45%{opacity:1}46%,100%{opacity:0}}
        @media(max-width:767px){.typing-word{white-space:normal}.typing-caret{width:2px;height:.82em;margin-left:4px}.gradient-mesh-bg .hero-grid{background-size:30px 30px}.gradient-mesh-bg h1{font-size:44px!important;line-height:1.02!important;letter-spacing:-.055em}.gradient-mesh-bg{background:radial-gradient(circle at 8% 12%,rgba(255,112,45,.10),transparent 28%),linear-gradient(180deg,#fff 0%,#fffaf7 100%)}}
      `}</style>
    </div>
  );
}

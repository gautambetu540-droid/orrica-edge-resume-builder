...
const FREE_FEATURES = [
  { icon: FileText, title: 'Resume Builder', desc: 'Create and edit resumes with guided sections, ready-made content ideas and a focused workspace.' },
  { icon: Upload, title: 'Resume Upload', desc: 'Bring an existing resume into your workflow and use it as the starting point for improvements.' },
  { icon: FileCheck2, title: 'Resume Check', desc: 'Review your resume for structure, readability and ATS-focused improvements before you apply.' },
  { icon: Target, title: 'ATS-Friendly Templates', desc: 'Choose professional layouts designed to keep important resume information easy to scan.' },
  { icon: Sparkles, title: 'AI Writing Assistance', desc: 'Improve summaries and achievement bullets using the information you provide, with AI kept optional.' },
  { icon: Download, title: 'PDF Export', desc: 'Preview your A4 resume and export a clean PDF when your application is ready.' },
];
...
const FAQS = [
  ...
  ['Is Orrica Edge free to use?', 'Orrica Edge is designed to be free to start. You can build and improve your resume without being forced into a paid workflow before you can create your document.'],
  ['Are Orrica Edge resume templates ATS-friendly?', 'Our templates use clean, text-first structures designed to keep important sections such as experience, education and skills easy for recruiting software and people to scan.'],
  ['Can I build a resume without AI?', 'Yes. AI is optional. You can write, edit and format every section manually and keep complete control over your final resume.'],
  ['Can I upload my existing resume?', 'Yes. The workflow supports starting from an existing resume so you can improve and customize information you already have instead of starting from zero.'],
  ['Can I customize the resume templates?', 'Yes. You can tailor content, section order, typography, spacing and accent styling while keeping the overall layout professional and readable.'],
  ['Can I create a resume if I am a fresher?', 'Yes. Orrica Edge can be used by freshers, students and experienced professionals. Guided sections help you present education, projects, skills, internships and achievements clearly.'],
  ...
  ['Does Orrica Edge work on mobile devices?', 'The experience is designed to be responsive so candidates can review and update their resume from desktop, tablet or mobile screens.'],
];
...
function ProductPreview() {
  return <div className="relative mx-auto w-full max-w-[760px] animate-scale-in reveal-4"><div className="absolute -inset-12 rounded-[56px] bg-orange-500/10 blur-3xl" /><div className="relative overflow-hidden rounded-[30px] border border-orange-100/70 bg-white shadow-[0_48px_120px_-54px_rgba(15,23,42,.45)]"><div className="flex h-12 items-center justify-between border-b border-neutral-100 bg-white px-4 sm:px-5"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-neutral-300" /><span className="h-2 w-2 rounded-full bg-neutral-300" /><span className="h-2 w-2 rounded-full bg-neutral-300" /><span className="ml-2 text-[11px] font-semibold text-neutral-500">Orrica Edge · Resume Builder</span></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[9px] font-bold text-emerald-700">Saved</span></div><div className="grid min-h-[470px] grid-cols-[132px_1fr] bg-neutral-50 sm:grid-cols-[170px_1fr]"><aside className="border-r border-neutral-100 bg-white p-3 sm:p-4"><div className="mb-4 px-2 text-[8px] font-bold uppercase tracking-[0.16em] text-neutral-400">Resume sections</div>{['Personal','Experience','Education','Skills','Projects','Summary'].map((item,index)=><div key={item} className={`mb-1 flex items-center gap-2 rounded-lg px-2 py-2 text-[9px] font-semibold ${index===0?'bg-orange-50 text-orange-700':'text-neutral-500'}`}><span className={`flex h-5 w-5 items-center justify-center rounded-full text-[7px] ${index===0?'bg-orange-500 text-white':'bg-neutral-100'}`}>{index===0?'✓':index+1}</span>{item}</div>)}<div className="mt-5 border-t border-neutral-100 pt-4"><div className="px-2 text-[8px] font-bold uppercase tracking-[0.16em] text-neutral-400">Tools</div><div className="mt-2 rounded-lg border border-orange-100 bg-orange-50/60 px-2 py-2 text-[9px] font-semibold text-orange-700">✦ Improve with AI</div></div></aside><div className="grid grid-cols-[1fr_.82fr] gap-3 p-3 sm:gap-4 sm:p-4"><div className="rounded-xl border border-neutral-100 bg-white p-3.5 shadow-sm sm:p-4"><div className="mb-4"><div className="text-sm font-bold text-neutral-900 sm:text-base">Personal information</div><div className="mt-1 text-[8px] text-neutral-400">Start with the details recruiters need.</div></div>{['Full name','Professional title','Email address','Phone number'].map((label,index)=><div key={label} className="mb-2.5"><div className="mb-1 text-[7px] font-bold text-neutral-500">{label}</div><div className="h-7 rounded-md border border-neutral-200 bg-white px-2 py-2 text-[8px] text-neutral-700">{['John Doe','Product Designer','john@example.com','+91 98765 43210'][index]}</div></div>)}<div className="mt-4 flex justify-end"><span className="rounded-md bg-orange-500 px-3 py-1.5 text-[8px] font-bold text-white">Continue →</span></div></div><div className="relative flex items-start justify-center rounded-xl border border-neutral-200 bg-neutral-100 p-2.5 sm:p-3"><div className="w-full bg-white p-3 shadow-md sm:p-4"><div className="border-b border-neutral-200 pb-3"><div className="text-[13px] font-black tracking-tight text-neutral-900">JOHN DOE</div><div className="mt-0.5 text-[6px] font-bold tracking-widest text-orange-600">PRODUCT DESIGNER</div><div className="mt-1 text-[5.5px] text-neutral-400">john@example.com · +91 98765 43210</div></div>{['EXPERIENCE','EDUCATION','SKILLS'].map(section=><div key={section} className="mt-3"><div className="text-[6px] font-black tracking-widest text-neutral-800">{section}</div><div className="mt-1 h-1 rounded bg-neutral-100"/><div className="mt-1 h-1 w-4/5 rounded bg-neutral-100"/><div className="mt-1 h-1 w-3/5 rounded bg-neutral-100"/></div>)}</div></div></div></div></div></div>;
}

function TemplateShowcase() {
  const templates = TEMPLATE_LIST.slice(0, 7);
  const scroll = (direction: number) => document.getElementById('oe-template-carousel')?.scrollBy({ left: direction * 360, behavior: 'smooth' });
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#fff8f1_0%,#fffaf7_52%,#fff3e8_100%)] py-12 text-neutral-900 sm:py-16 lg:py-20"><div className="template-showcase-inner relative mx-auto max-w-[1500px] px-5 sm:px-8 lg:px-10"><div className="mx-auto max-w-[820px] text-center"><h2 className="font-serif text-3xl font-bold tracking-[-0.035em] text-neutral-950 sm:text-4xl lg:text-[50px]">Professional Resume Templates</h2><p className="mx-auto mt-3 max-w-[720px] text-sm font-medium leading-6 text-orange-800/80 sm:text-[15px]">Choose an ATS-friendly, recruiter-ready <Link href="/templates" className="font-semibold text-orange-700 underline decoration-orange-200 decoration-2 underline-offset-4 hover:text-orange-800">resume template</Link> and customize it for the role you want.</p></div><div className="relative mt-9 sm:mt-11"><button type="button" aria-label="Previous templates" onClick={() => scroll(-1)} className="absolute left-1 top-1/2 z-20 hidden h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-orange-200 bg-white text-neutral-700 shadow-lg transition hover:-translate-x-0.5 hover:text-orange-600 md:flex">←</button><div id="oe-template-carousel" className="flex snap-x gap-5 overflow-x-auto pb-3 pr-2 scrollbar-none md:px-12">{templates.map((template) => <Link href={`/resume/new?template=${template.id}`} key={template.id} className="group min-w-[210px] snap-start overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-[0_20px_45px_-35px_rgba(249,115,22,.32)] transition hover:-translate-y-1 hover:border-orange-300"><div className="aspect-[0.77] overflow-hidden bg-neutral-100 p-2.5"><TemplatePreviewCard template={template} accent={template.defaultAccentColor} layout={template.layout} headerVariant={template.headerVariant} sidebarVariant={template.sidebarVariant} font={template.recommendedFont} /></div><div className="p-4"><div className="text-sm font-bold text-neutral-950">{template.name}</div><div className="mt-1 text-[11px] text-neutral-500">ATS-friendly · Ready to customize</div><div className="mt-3 text-xs font-bold text-orange-700">Use template →</div></div></Link>)}</div><button type="button" aria-label="Next templates" onClick={() => scroll(1)} className="absolute right-1 top-1/2 z-20 hidden h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-orange-200 bg-white text-neutral-700 shadow-lg transition hover:translate-x-0.5 hover:text-orange-600 md:flex">→</button></div><div className="mt-7 flex justify-center"><Link href="/templates" className="inline-flex h-10 items-center justify-center rounded-lg bg-orange-600 px-5 text-sm font-bold text-white shadow-[0_12px_28px_-18px_rgba(249,115,22,.55)] transition hover:-translate-y-0.5 hover:bg-orange-700">View all resume templates <ArrowRight className="ml-2 h-4 w-4" /></Link></div></div></section>
  );
}

function FreeAccessSection() {
  return (
    <section className="border-y border-orange-100 bg-white py-20 sm:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-end"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-600">Create a FREE Resume</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-neutral-950 sm:text-5xl">Get access to powerful resume-building tools.</h2></div><p className="max-w-2xl text-sm leading-7 text-neutral-500 lg:justify-self-end">Build, improve, check and export your resume through one focused workflow. Start with your real information, choose your design and keep control of every final change.</p></div><div className="mt-12 grid gap-px overflow-hidden rounded-3xl border border-orange-100 bg-orange-100 sm:grid-cols-2 lg:grid-cols-3">{FREE_FEATURES.map((feature) => <article key={feature.title} className="group bg-white p-7 transition hover:bg-orange-50/40"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-orange-50 text-orange-600 transition group-hover:scale-105 group-hover:bg-orange-100"><feature.icon className="h-5 w-5" /></div><h3 className="mt-5 text-sm font-bold text-neutral-950">{feature.title}</h3><p className="mt-2 text-sm leading-6 text-neutral-500">{feature.desc}</p></article>)}</div><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Link href="/resume/new"><Button size="lg" className="h-11 rounded-xl bg-orange-600 px-6 text-sm font-bold text-white hover:bg-orange-700">Build New Resume <ArrowRight className="ml-1 h-4 w-4" /></Button></Link><Link href="/templates"><Button size="lg" variant="outline" className="h-11 rounded-xl border-orange-200 bg-white px-6 text-sm font-bold text-orange-700 hover:bg-orange-50">Explore Templates</Button></Link></div></div></section>
  );
}

function CareerPartnerSection() {
  return (
    <section className="border-y border-black/[0.06] bg-neutral-50 py-20 sm:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-600">Your Strategic Career Partner</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-neutral-950 sm:text-5xl">Practical guidance for every step of the application.</h2><p className="mt-4 text-base leading-7 text-neutral-500">Orrica Edge brings resume writing, job-application preparation and career resources into one simple experience.</p></div><div className="mt-12 grid gap-4 lg:grid-cols-4">{[['Expertise you can trust','Clear, practical resume guidance built around strong content and professional presentation.'],['Industry-aware advice','Role-focused tips to help you highlight the skills, projects and experience that matter for your target job.'],['Practical strategies','Actionable help for resumes, interviews, applications and improving your chances of being noticed.'],['Clear and useful resources','Simple career content you can use immediately without getting lost in unnecessary complexity.']].map(([title,desc])=><article key={title} className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-[0_16px_40px_-32px_rgba(15,23,42,.25)]"><div className="h-1.5 w-12 rounded-full bg-orange-500"/><h3 className="mt-5 text-base font-bold text-neutral-950">{title}</h3><p className="mt-2 text-sm leading-6 text-neutral-500">{desc}</p></article>)}</div></div></section>
  );
}
...
function CandidateFeedbackSection() {
  ...
  return (
    <section id="feedback" className="relative overflow-hidden border-y border-orange-100 bg-[linear-gradient(180deg,#fffaf6_0%,#ffffff_55%,#fff7f0_100%)] py-20 sm:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="mx-auto max-w-3xl text-center"><div className="inline-flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-orange-700"><Sparkles className="h-3.5 w-3.5" /> Candidate feedback</div><h2 className="mt-4 text-3xl font-semibold tracking-[-0.045em] text-neutral-950 sm:text-5xl">What candidates say after building their resume.</h2></div>{items.length > 0 ? <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{items.map((item) => <article key={item.id} className="rounded-3xl border border-orange-100 bg-white p-6 shadow-sm"><div className="flex items-center gap-1" aria-label={`${item.rating} out of 5 stars`}>{Array.from({ length: 5 }).map((_, index) => <Star key={index} className={`h-4 w-4 ${index < item.rating ? 'fill-orange-400 text-orange-400' : 'text-neutral-200'}`} />)}</div><p className="mt-4 text-[15px] leading-7 text-neutral-700">{item.feedback}</p><div className="mt-5 text-xs font-bold text-neutral-950">{item.candidateName || 'Candidate'}</div></article>)}</div> : <div className="mx-auto mt-12 max-w-4xl rounded-[28px] border border-dashed border-neutral-300 bg-neutral-50 p-8 text-center"><h3 className="text-lg font-bold text-neutral-950">Your feedback can be featured here.</h3><p className="mt-2 text-sm text-neutral-500">Once candidates share feedback, selected reviews will appear here.</p></div>}</div></section>
  );
}

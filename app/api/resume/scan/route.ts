...
const ImportedResumeSchema = z.object({
  ...
  achievements: z.array(z.object({
    type: z.enum(['award', 'achievement', 'publication', 'volunteer', 'other']),
    title: z.string(),
    description: z.string().optional(),
...
function normalize(data: z.infer<typeof ImportedResumeSchema>) {
  ...
  return {
    ...
    skills: (['technical', 'soft', 'tools', 'languages'] as const).map((category) => ({
      category,
...
export async function POST(request: NextRequest) {
  if (!process.env.OPENAI_API_KEY) {
    return NextResponse.json({ error: 'AI scanning is not configured. Add OPENAI_API_KEY to the server environment.' }, { status: 503 });
  }
  ...
  try {
    ...
    if (text.length < 80) {
      return NextResponse.json({
        error: 'This PDF looks like a scanned/image-only resume. Please upload a text-based PDF for automatic import, or export the scan as a searchable PDF first.',
      }, { status: 422 });
    }
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `Scan this existing resume text and map it into the resume builder. Do not rewrite or improve it yet; this step is extraction only.\n\nRESUME TEXT:\n${text.slice(0, 50000)}`,
        },
      ],
    });
    ...
    const result = ImportedResumeSchema.parse(parsedJson);
    return NextResponse.json({ data: normalize(result), source: 'ai-pdf-scan' });
  } catch (error) {
    console.error('Resume scan error:', error);
    const message = error instanceof Error ? error.message : 'Could not scan this resume.';

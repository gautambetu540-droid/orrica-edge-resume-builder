...
const noStore = (body: unknown, status = 200) => NextResponse.json(body, { status, headers: { 'Cache-Control': 'private, no-store, max-age=0', 'X-Content-Type-Options': 'nosniff' } });
...
const text = (v: unknown): string => { if (typeof v === 'string') return v.trim(); if (v == null) return ''; if (Array.isArray(v)) return v.flat(Infinity).map(text).filter(Boolean).join('\n'); if (typeof v === 'object') return Object.values(v as Record<string, unknown>).map(text).filter(Boolean).join('\n'); return String(v).trim(); };
...
const enumValue = (v: unknown, allowed: readonly string[], fallback: string) => { const s = text(v).toLowerCase().replace(/[\s_-]/g, ''); const aliases: Record<string, string> = { beginner: 'basic', elementary: 'basic', intermediate: 'conversational', upperintermediate: 'professional', advanced: 'fluent' }; return allowed.find(x => x.replace(/_/g, '') === s) || aliases[s] || fallback; };
...
function normalizeRaw(input: unknown) {
  ...
  const skills = Array.isArray(rawSkills) ? rawSkills.map(x => { const o = x && typeof x === 'object' ? x as Record<string, unknown> : {}; return { category: text(o.category).toLowerCase(), items: list(o.items ?? o.skills ?? x) }; }) : rawSkills && typeof rawSkills === 'object' ? Object.entries(rawSkills as Record<string, unknown>).map(([category, value]) => ({ category: category.toLowerCase(), items: list(value) })) : [];
  return {
    personalInfo: { fullName: text(p.fullName ?? p.full_name ?? raw.fullName ?? raw.name), professionalTitle: text(p.professionalTitle ?? p.title ?? raw.professionalTitle ?? raw.title), email: text(p.email ?? raw.email), phone: text(p.phone ?? p.mobile ?? p.mobileNumber ?? raw.phone ?? raw.mobile ?? raw.mobileNumber), city: text(p.city ?? raw.city), country: text(p.country ?? raw.country), linkedin: text(p.linkedin ?? raw.linkedin), portfolio: text(p.portfolio ?? raw.portfolio), github: text(p.github ?? raw.github) },
    summary: text(raw.summary ?? raw.profile ?? raw.objective),
    experience: arr(raw.experience).map(x => { const o = x && typeof x === 'object' ? x as Record<string, unknown> : {}; return { company: text(o.company ?? o.employer ?? o.organization), jobTitle: text(o.jobTitle ?? o.title ?? o.position ?? o.role), location: text(o.location), startDate: text(o.startDate ?? o.from ?? o.start), endDate: text(o.endDate ?? o.to ?? o.end), currentlyWorking: Boolean(o.currentlyWorking ?? o.current ?? o.present), responsibilities: text(o.responsibilities ?? o.responsibility ?? o.duties ?? o.description), achievements: list(o.achievements ?? o.accomplishments) }; }),
    education: arr(raw.education).map(x => { const o = x && typeof x === 'object' ? x as Record<string, unknown> : {}; return { institution: text(o.institution ?? o.school ?? o.university ?? o.college), degree: text(o.degree ?? o.qualification), fieldOfStudy: text(o.fieldOfStudy ?? o.field ?? o.specialization), startDate: text(o.startDate ?? o.from ?? o.start), endDate: text(o.endDate ?? o.to ?? o.end), grade: text(o.grade ?? o.cgpa ?? o.percentage), description: text(o.description) }; }),
...
function addIds(data: z.infer<typeof Schema>) { const id = () => Math.random().toString(36).slice(2, 10); const map = new Map(data.skills.map(s => [s.category, s.items])); return { ...data, experience: data.experience.map(x => ({ ...x, id: id() })), education: data.education.map(x => ({ ...x, id: id() })), skills: categories.map(c => ({ category: c, items: map.get(c) || [] })), projects: data.projects.map(x => ({ ...x, id: id() })), certifications: data.certifications.map(x => ({ ...x, id: id() })), languages: data.languages.map(x => ({ ...x, id: id() })), achievements: data.achievements.map(x => ({ ...x, id: id() })) }; }
...
export async function POST(request: NextRequest) {
  ...
  const rate = checkRateLimit(`resume-scan:${user.id}`); if (!rate.allowed) return noStore({ error: 'Too many scan attempts. Please wait a moment and try again.' }, 429);
  const apiKey = process.env.GEMINI_API_KEY?.trim(); if (!apiKey) return noStore({ error: 'Resume scanning is not configured on the server.' }, 503);
  try {
    ...
    let resumeText = ''; try { resumeText = (await extractPdfText(buffer)).text; } catch (e) { console.warn('PDF text extraction failed; using Gemini PDF vision fallback.', e); }

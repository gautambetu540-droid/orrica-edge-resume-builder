  ...
  try {
    ...
    if (!experience.length && !education.length) {
      return NextResponse.json(
        {
          error:
            'Add at least one work experience or education entry before generating a summary.',
        },
        { status: 400 }
      );
    }
    ...
    const toneInstruction = {
      ...
      concise:
        'Write a tight, concise 2-sentence summary.',
      ats:
        'Write in ATS-friendly language, front-loading relevant keywords from the person’s actual background.',
    }[tone];
...
Output ONLY the summary text.`,
          },
          {
            ...
            content: `${toneInstruction}${
              targetRole
                ? ` Tailor it toward this target role: ${targetRole}.`
                : ''
            }

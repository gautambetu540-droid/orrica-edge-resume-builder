...
const RequestSchema = z.object({
  ...
  jobDescription: z.string().min(20, 'Paste the full job description (at least 20 characters).'),
});
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { allowed, retryAfterMs } = checkRateLimit(`job-match:${rateKey}`);
    if (!allowed) {
...
- Be realistic with matchScore; do not inflate it.`,
    ...
    return NextResponse.json({ error: 'Something went wrong analyzing this job description. Please try again.' }, { status: 500 });
  }
}

...
const RequestSchema = z.object({
  ...
  context: z.string().optional().default(''), // e.g. job title, company, for better grounding
});

const ACTION_INSTRUCTIONS: Record<(typeof ACTIONS)[number], string> = {
  rewrite: 'Rewrite this content to be clearer and more compelling, preserving every fact exactly.',
  'make-professional': 'Rewrite this in a more polished, professional tone suitable for a corporate resume.',
  'make-concise': 'Make this significantly more concise while keeping all facts and key points.',
  'ats-optimize':
  ...
  shorten: 'Shorten this to roughly half its current length while preserving the key facts.',
  'fix-grammar': 'Fix grammar, spelling, and punctuation only. Do not change the meaning, tone, or facts.',
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { allowed, retryAfterMs } = checkRateLimit(`improve-section:${rateKey}`);
    if (!allowed) {
    ...
    const { section, content, action, targetRole, context } = parsed.data;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `${CORE_RULES}\n\nYou are editing the "${section}" section of a resume. Output ONLY the improved text — no preamble, no quotes, no explanation.`,
        },
        {
          role: 'user',
    ...
    return NextResponse.json({ original: content, improved });
  } catch (err) {
    console.error('improve-section error:', err);
    return NextResponse.json({ error: 'Something went wrong improving this section. Please try again.' }, { status: 500 });
  }
}

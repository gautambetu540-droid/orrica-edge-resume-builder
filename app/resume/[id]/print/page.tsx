...
export default async function PrintPage({ params }: { params: { id: string } }) {
  ...
  const data = { ...EMPTY_RESUME_DATA, ...resume.resume_data };
  const settings = { ...DEFAULT_SETTINGS, ...resume.settings, template: resume.template ?? DEFAULT_SETTINGS.template };
  ...
  return (
    <div style={{ margin: 0, background: 'white', minHeight: '100vh' }}>
      <ResumeDocument data={data} settings={settings} forPrint />
    </div>
  );
}
